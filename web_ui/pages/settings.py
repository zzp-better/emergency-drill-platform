...
def _render_cluster_tab():
    ...
    if profile_names:
        default_idx = next(
            (i for i, p in enumerate(k8s_profiles) if p.get("is_default")), 0
        )
        selected_profile = st.selectbox(
            "选择配置档案", profile_names, index=default_idx
        )
    ...
    connection_type = st.radio(
        ...
        index=(
            0
            if cluster_config.get("connection_type", "kubeconfig") == "kubeconfig"
            else 1
        ),
        horizontal=True,
    )
    ...
    if connection_type == "kubeconfig":
        ...
        if ep:
            st.caption(
                ("✓ 文件存在: " if os.path.exists(ep) else "✗ 文件不存在: ") + ep
            )
    else:
    ...
    with col_d:
        if selected_profile and st.button(
            "🗑 删除档案", key="del_k8s_profile", use_container_width=True
        ):
    ...
    with col_def:
        if selected_profile and st.button(
            "⭐ 设为默认", key="k8s_set_default_btn", use_container_width=True
        ):
...
def _render_monitor_tab():
    ...
    if mon_names:
        default_mon_idx = next(
            (i for i, p in enumerate(mon_profiles) if p.get("is_default")), 0
        )
    ...
    with col2:
        ...
        password = st.text_input(
            "密码",
            value=monitor_config.get("password", ""),
            type="password",
            placeholder="可选（保存监控档案时会一起保存）",
        )
    ...
    with col_ma:
        if st.button(
            ...
            try:
                set_monitor_checker(
                    MonitorChecker(prometheus_url, username, password or None)
                )
                monitor_config["prometheus_url"] = prometheus_url
                monitor_config["username"] = username
                monitor_config["password"] = password
                st.success("✓ 监控已连接")
                st.rerun()
    ...
    with col_ms:
        if st.button("💾 保存监控档案", key="mon_save_btn", use_container_width=True):
            ...
            if n:
                ...
                st.warning("请输入档案名称")
    with col_md:
        if selected_mon and st.button(
            "🗑 删除档案", key="del_monitor_profile", use_container_width=True
        ):
    ...
    with col_mdef:
        if selected_mon and st.button(
            "⭐ 设为默认", key="mon_set_default_btn", use_container_width=True
        ):
...
def _render_notify_tab():
    ...
    with col_nsave:
        if st.button(
            "💾 保存通知配置", key="notify_save_btn", use_container_width=True
        ):
            db.save_notify_config(notify_config)
            st.success("✓ 通知配置已保存")
    with col_ntest:
        if webhook_url and st.button(
            "🧪 测试通知", key="notify_test_btn", use_container_width=True
        ):
            try:
                import requests as req

                payload = {"text": "🔔 这是一条测试通知，来自应急演练平台"}
...
def _render_schedule_tab():
    ...
    if schedules:
        for s in schedules:
            ...
            with st.expander(
                f"{enabled_icon} {s['name']}  |  `{s['cron_expr']}`  |  {s['scenario']}"
            ):
                ...
                with col3:
                    if st.button(
                        "▶️ 启用" if not s.get("enabled") else "⏸️ 禁用",
    ...
    with sn_col2:
        cron_expr = st.text_input(
            "Cron 表达式", placeholder="0 * * * *", key="cron_expr"
        )
    ...
    with col3:
        pod_selector = st.text_input(
            "Pod 选择器", placeholder="pod-name-prefix", key="pod_selector"
        )

    if st.button(
        "➕ 添加计划", key="schedule_add_btn", use_container_width=True, type="primary"
    ):

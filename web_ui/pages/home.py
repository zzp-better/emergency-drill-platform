...
from ..demo_mode import (
    render_demo_flow,
    render_demo_quick_actions,
    render_environment_status_cards,
    render_standard_demo_entry,
)
...
def render():
    ...
    # 页面标题
    st.markdown(
        '<div class="main-header">应急演练智能平台</div>', unsafe_allow_html=True
    )
    st.markdown("### 🏠 平台概览")
    ...
    cluster_ready = get_chaos_injector() is not None
    monitor_ready = get_monitor_checker() is not None
    running_tasks = {
        k: v for k, v in drill_tasks.items() if v.get("status") == "running"
    }
    ...
    demo_col, flow_col = st.columns([1.25, 1])
    with demo_col:
        render_standard_demo_entry(cluster_ready, monitor_ready)
    with flow_col:
        render_demo_flow(cluster_ready, monitor_ready)
    ...
    if upcoming:
        with st.expander(
            f"即将执行的演练计划（{len(upcoming)} 条）", expanded=bool(upcoming)
        ):
            for s in upcoming:
    ...
    if history:
        ...
        for item in recent:
            ...
            status_class = (
                "status-success" if item.get("status") == "成功" else "status-error"
            )
            st.markdown(
                f"""

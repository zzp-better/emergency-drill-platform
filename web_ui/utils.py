...
def validate_input(input_str: str, field_name: str, max_length: int = 253) -> tuple[bool, str]:
    """
    验证用户输入
...
def build_injector(config: Optional[Dict] = None, use_chaos_mesh: bool = False) -> Optional[ChaosInjector]:
    """
    ...
    try:
        ...
        if conn_type == 'token':
            ...
            kubeconfig_path = config.get('kubeconfig_path', os.path.expanduser('~/.kube/config'))
...
def init_monitor_checker(prometheus_url: str, username: Optional[str] = None, password: Optional[str] = None) -> bool:
    """
    初始化监控验证器
...
def generate_drill_report(scenario, namespace, pod_name, start_time, end_time, drill_duration, result):
    """

...
from ..demo_templates import render_demo_chain_templates
from ..state import (
    init_session_state,
    get_chaos_injector,
    get_drill_tasks,
...
def render():
            ...
            for chain in chains:
                with st.expander(f"🔗 {chain['name']}", expanded=False):
                    ...
                    for idx, stage in enumerate(stages):
                        ...
                        if stype == "fault":
                            st.write(
                                f"  {idx+1}. ⚡ 故障注入: {stage.get('scenario', 'unknown')}"
                            )
                        elif stype == "wait":
                            st.write(
                                f"  {idx+1}. ⏳ 等待: {stage.get('wait_seconds', 0)}秒"
                            )
                        elif stype == "verify_alert":
                            st.write(
                                f"  {idx+1}. 🔔 验证告警: {stage.get('alert_name', 'unknown')}"
                            )
                    ...
                    with btn_col1:
                        if st.button(
                            ...
                            _start_chain_execution(
                                chain, injector, drill_tasks, drill_tasks_lock
                            )
                    with btn_col2:
                        if st.button(
                            f"🗑️ 删除",
    ...
    with col2:
        st.subheader("➕ 创建新故障链")
        render_demo_chain_templates()
        st.markdown("---")

        chain_name = st.text_input(
            "故障链名称", placeholder="例如：CPU+网络组合故障", key="chain_name"
        )
        chain_desc = st.text_area(
            "描述", placeholder="描述这个故障链的目的...", key="chain_desc", height=80
        )
        ...
        if st.session_state.current_stages:
            ...
            for idx, stage in enumerate(st.session_state.current_stages):
                ...
                if stype == "fault":
                    st.write(
                        f"  {idx+1}. ⚡ 故障注入: {stage.get('scenario', 'unknown')} ({stage.get('duration', 60)}秒)"
                    )
                elif stype == "wait":
                    st.write(f"  {idx+1}. ⏳ 等待: {stage.get('wait_seconds', 10)}秒")
                elif stype == "verify_alert":
                    st.write(
                        f"  {idx+1}. 🔔 验证告警: {stage.get('alert_name', '')} 期望={stage.get('expected', 'firing')}"
                    )
            ...
            for idx, stage in enumerate(st.session_state.current_stages):
                ...
                with c2:
                    if st.button("⬆️", key=f"up_{idx}", disabled=(idx == 0)):
                        (
                            st.session_state.current_stages[idx],
                            st.session_state.current_stages[idx - 1],
                        ) = (
                            st.session_state.current_stages[idx - 1],
                            st.session_state.current_stages[idx],
                        )
                ...
                with c3:
                    if st.button(
                        "⬇️",
                        key=f"down_{idx}",
                        disabled=(idx == len(st.session_state.current_stages) - 1),
                    ):
                        (
                            st.session_state.current_stages[idx],
                            st.session_state.current_stages[idx + 1],
        ...
        stage_type = st.selectbox(
            "步骤类型", ["故障注入", "等待", "验证告警"], key="new_stage_type"
        )

        if stage_type == "故障注入":
            scenario = st.selectbox(
                "故障场景", list(SCENARIO_MAP.keys()), key="fault_scenario"
            )
            namespace = st.text_input("命名空间", value="default", key="fault_ns")
            pod_selector = st.text_input(
                "Pod 名称", placeholder="目标 Pod 名称", key="fault_pod"
            )
            duration = st.number_input(
                "持续时间(秒)", min_value=10, value=60, key="fault_duration"
            )

            # 根据场景类型显示额外参数
            extra_params = {}
            if scenario == "cpu_stress":
                extra_params["cpu_workers"] = st.number_input(
                    "CPU Workers", min_value=1, value=1, key="cpu_workers"
                )
                extra_params["cpu_load"] = st.number_input(
                    "CPU 负载(%)", min_value=1, max_value=100, value=100, key="cpu_load"
                )
            elif scenario == "memory_stress":
                extra_params["memory_size"] = st.text_input(
                    "内存大小", value="256Mi", key="mem_size"
                )
                extra_params["memory_workers"] = st.number_input(
                    "Memory Workers", min_value=1, value=1, key="mem_workers"
                )
            elif scenario == "network_delay":
                extra_params["latency"] = st.text_input(
                    "延迟", value="100ms", key="net_latency"
                )
                extra_params["jitter"] = st.text_input(
                    "抖动", value="10ms", key="net_jitter"
                )
            elif scenario == "disk_io":
                extra_params["path"] = st.text_input(
                    "磁盘路径", value="/var/log", key="disk_path"
                )
                extra_params["size"] = st.text_input(
                    "填充大小", value="1Gi", key="disk_size"
                )
            ...
            wait_seconds = st.number_input(
                "等待时间(秒)", min_value=1, value=30, key="wait_sec"
            )
            if st.button("➕ 添加等待步骤", use_container_width=True):
                new_stage = {"type": "wait", "wait_seconds": wait_seconds}
                st.session_state.current_stages.append(new_stage)
                st.rerun()

        else:  # 验证告警
            alert_name = st.text_input(
                "告警名称", placeholder="例如: PodCrashLoopingOff", key="alert_name"
            )
            expected = st.selectbox(
                "期望状态", ["firing", "resolved"], key="alert_expected"
            )
        ...
        with c1:
            if st.button("💾 保存故障链", type="primary", use_container_width=True):
                if not chain_name:
                    ...
                    db.save_drill_chain(
                        chain_name, chain_desc, st.session_state.current_stages
                    )
                    st.session_state.current_stages = []
